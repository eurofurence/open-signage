...
const groupedSchedule = computed(() => {
  ...
  return schedule.value
    ...
    .reduce((grouped, entry) => {
      const date = entry.ends_at.split('T')[0];
      if (!grouped[date]) {
        grouped[date] = [];
      }
...
<template>
  <div class="bg-primary-400 min-h-screen">
    <component
      :is="returnDivOrComponent(Carousel)"
      snapAlign="start"
      class=""
      :itemsToShow="showItemsBasedOnScreenSize"
      :center-mode="false"
    >
      <component
        :is="returnDivOrComponent(Slide)"
        class="first:border-l-0 block"
        :key="dayIndex"
                ...
                <div class="p-2 text-left">
                  ...
                  <div class="pb-1 text-primary-950">
                    <div class="flex justify-between gap-3">
                      <div class="text-sm font-bold">
                        <span v-if="panel.flags.find(f => f === 'after_dark')">[After Dark] </span>{{ panel.title }}
                      </div>
                      <div class="text-white text-sm">
                        ...
                        <div v-else-if="panel.flags.find(e => e === 'cancelled')" class="rounded">
                          <div class="font-mono px-1 font-bold text-center w-full bg-black rounded">CANCELLED</div>
                        </div>
                        <div v-else-if="panel.flags.find(e => e === 'moved')" class="rounded">
                          <div class="font-mono px-1 font-bold text-center w-full bg-yellow-600 rounded">MOVED</div>
                        </div>
                        <div v-else-if="panel.delay && !entryInPast(panel)" class="rounded">
                          <div class="font-mono px-1 font-bold text-center w-full bg-red-900 rounded">DELAYED</div>
                        </div>
                      </div>
                    </div>
                    <div v-if="panel.schedule_organizer" class="text-xs">
                      Organized by
                  ...
                  <div>
                    <div class="flex justify-between text-sm font-semibold">
                      <div>
                        <div
                          v-if="panel.delay > 0 && !entryInPast(panel)"
                        ...
                        <div
                          :class="{
                            'font-bold': panel.delay > 0 && !entryInPast(panel),
                          }"
                        >
                  ...
                  <div v-if="panel.description" class="text-xs mt-1">
                    {{ panel.description }}
                  </div>
                  <div v-if="panel.message" class="text-xs mt-2 p-1 rounded bg-white font-semibold">
                    {{ panel.message }}
                  </div>
                </div>
              </div>
